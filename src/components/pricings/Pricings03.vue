<template>
<section class="fdb-block bg-dark" style="background-image: url(imgs/bg_0.svg);">
  <div class="container">
    <div class="row text-center">
      <div class="col">
        <h1>Pricing</h1>
      </div>
    </div>

    <div class="row mt-5 align-items-center">
      <div class="col-12 col-sm-10 col-md-8 m-auto col-lg-4 text-center">
        <div class="fdb-box fdb-touch p-4 rounded">
          <h2>Hobby</h2>
          <p class="h3">Far far away, behind the word mountains, far from the countries Vokalia and Consonantia.</p>

          <p class="h1 mt-5 mb-5">$99</p>

          <p><a href="https://www.froala.com" class="btn btn-primary">Buy Now</a></p>
        </div>
      </div>

      <div class="col-12 col-sm-10 col-md-8 m-auto col-lg-4 text-center pt-4 pt-lg-0">
        <div class="fdb-box fdb-touch p-4 rounded">
          <h2>Advanced</h2>
          <p class="h3">Separated they live in Bookmarksgrove right at the coast, a large language ocean.</p>

          <p class="h1 mt-5 mb-5">$299</p>

          <p><a href="https://www.froala.com" class="btn btn-primary">Buy Now</a></p>
        </div>
      </div>

      <div class="col-12 col-sm-10 col-md-8 m-auto col-lg-4 text-center pt-4 pt-lg-0">
        <div class="fdb-box fdb-touch p-4 rounded">
          <h2>Business</h2>
          <p class="h3">Even the all-powerful Pointing has no control about the blind texts it is an almost unorthographic life.</p>

          <p class="h1 mb-5 mt-5">$799</p>

          <p><a href="https://www.froala.com" class="btn btn-primary">Buy Now</a></p>
        </div>
      </div>
    </div>
  </div>
</section>

</template>
